...
Эмулируется:
* два процессора КМ1801ВМ2, в процессоре пока 1 команда = 24 такта, т.е. без точного тайминга;
* контроллеры памяти;
* экран (практически полностью);
* клавиатура (нужно ещё добить раскладки);
* кассеты ПЗУ;
* дисководы.

НЕ эмулируется:
* звук (код написан но мешает отсутствие точного тайминга);
* стык С2;
* параллельный порт и принтер;
* магнитофон;
* сеть.
