...
Эмулятор написан на C++, библиотека — голый Win32, с Юникодом.
Среда разработки — Visual Studio 2005.

== Состояние эмуляции ==

Эмулируется: оба процессора, контроллеры памяти, экран (почти полностью), звук,
клавиатура (нужно ещё добить раскладки), кассеты ПЗУ, дисководы (пока только
80-дорожечные диски).
В процессоре пока 1 команда = 1 такт, т.е. без точного тайминга. 
Не эмулируется: стык С2, параллельный порт, магнитофон, сеть.
...
* Stop (мультимедийная кнопка) — то же что пункт меню '''Emulator | Reset'''
  — сброс УКНЦ.
